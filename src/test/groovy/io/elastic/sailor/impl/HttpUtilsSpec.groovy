...
class HttpUtilsSpec extends Specification {
    ...
    def "should post json successfully when credentials are inside url"() {
        ...
        setup:
        def url = "http://homer.simpson%40example.org:secret@localhost:12345/v1/exec/result/55e5eeb460a8e2070000001e"
        ...
        def result = HttpUtils.postJson(
                ...
                httpClient,
                body,
                new HttpUtils.BasicAuthorizationHandler(url))
    ...
    def "should fail to post json if user info not present in the url"() {
        ...
        when:
        def url = "http://localhost:10000/v1/exec/result/55e5eeb460a8e2070000001e"
        HttpUtils.postJson(
                url,
                httpClient,
                Json.createObjectBuilder().build(),
                new BasicAuthorizationHandler(url))
        then:
        def e = thrown(RuntimeException)
